...
Read the following instructions carefullly before getting into the game!!!
...
parser.add_argument("--sound-dir", type=str, default="./sounds")
...
GAME_MODES = {
    "classic": {
        "time": 60, "green_min": 1.8, "green_max": 5.0, "red_min": 2.0,
        "red_max": 4.5, "torso_easy": 0.035, "torso_hard": 0.012,
        "grace": 0.60, "label": "CLASSIC"
    },
    "hardcore": {
        "time": 45, "green_min": 1.2, "green_max": 3.0, "red_min": 2.5,
        "red_max": 5.5, "torso_easy": 0.025, "torso_hard": 0.008,
        "grace": 0.40, "label": "HARDCORE"
    },
    "training": {
        "time": 90, "green_min": 3.0, "green_max": 6.0, "red_min": 1.5,
...
RED_MIN = 2.0
RED_MAX = 4.5
TORSO_THRESH_EASY = 0.035
TORSO_THRESH_HARD = 0.012
GRACE_PERIOD = 0.60
...
def apply_mode(m):
    ...
    GAME_TIME = c["time"]
    GREEN_MIN = c["green_min"]
    GREEN_MAX = c["green_max"]
    RED_MIN = c["red_min"]
    RED_MAX = c["red_max"]
    TORSO_THRESH_EASY = c["torso_easy"]
    TORSO_THRESH_HARD = c["torso_hard"]
    GRACE_PERIOD = c["grace"]
    CURRENT_MODE = m
...
if DETECTOR == "yolo" and (not _HAS_YOLO or not os.path.exists(MODEL_PATH)):
    if _HAS_MP:
        logger.warning("YOLO unavailable, using MediaPipe.")
        DETECTOR = "mp"
    else:
        raise SystemExit("[ERROR] No detector available.")
...
def get_sound_path(key):
    ...
    path = SOUND_DIR / filename_map.get(key, f"{key}.wav")
    return path if path.exists() else None


SOUNDS = {k: get_sound_path(k) for k in [
    "rules", "background", "red", "green", "gunshot", "eliminated", "gameover"
]}

missing_sounds = [k for k, v in SOUNDS.items() if v is None]
if missing_sounds:
    logger.warning(f"Missing sound files: {', '.join(missing_sounds)}")

def col_green():
    return (180, 180, 0) if COLORBLIND_MODE else (30, 200, 30)


def col_red():
    return (255, 100, 0) if COLORBLIND_MODE else (0, 0, 220)
...
def col_box_g():
    return (180, 180, 0) if COLORBLIND_MODE else (50, 220, 50)


def col_box_r():
    return (255, 100, 0) if COLORBLIND_MODE else (50, 50, 220)
...
def _init_shapes(w, tx, tw, th):
    """Initialize animated header shapes."""
    global _shapes_line
    _shapes_line = []
    gl = max(0, tx - 22)
    gr = min(w, tx + tw + 22)
    ...
    if tot <= 50:
        return
    sz = max(10, int(th * 0.95))
    cy = HEADER_HEIGHT // 2
    step = tot / SHAPES_TOTAL
    prev = None
    cur = step / 2
...
def _draw_sh(img, st, cx, cy, sz, col=(0, 0, 0)):
    ...
    h2 = sz // 2
    if st == "circle":
        cv2.circle(img, (cx, cy), max(4, h2), col, -1, cv2.LINE_AA)
    elif st == "square":
        cv2.rectangle(img, (cx - h2, cy - h2), (cx + h2, cy + h2), col, -1, cv2.LINE_AA)
    else:  # triangle
...
def _render_shapes(img, tx, tw):
    """Render animated shapes in header."""
    if not _shapes_line:
        return
    hdr = img[0:HEADER_HEIGHT, :, :].copy()
    gl = max(0, tx - 22)
    gr = min(img.shape[1], tx + tw + 22)
    now = time.time()
    for s in _shapes_line:
        ...
        ly = hdr.copy()
        _draw_sh(ly, s["type"], s["cx"], s["cy"], s["size"])
        hdr = cv2.addWeighted(ly, a, hdr, 1 - a, 0)
    img[0:HEADER_HEIGHT, :, :] = hdr


def draw_header(img):
    """Draw header with title and mode info."""
    h, w = img.shape[:2]
...
class Audio:
    """Thread-safe audio playback handler."""
    ...
    def play_once(self, path):
        """Play sound effect once (non-blocking)."""
        if not SOUND_ENABLED or path is None:
            return
        try:
            sa.WaveObject.from_wave_file(str(path)).play()
        except Exception as e:
            logger.debug(f"Failed to play {path}: {e}")

    def play_blocking(self, path, timeout=AUDIO_TIMEOUT):
        """Play sound blocking with timeout."""
        if not SOUND_ENABLED or path is None:
            return
        try:
            wav = sa.WaveObject.from_wave_file(str(path))
            play_obj = wav.play()
            # Wait with timeout
            start = time.time()
            while play_obj.is_playing():
                if time.time() - start > timeout:
                    logger.warning(f"Audio playback timeout for {path}")
                    play_obj.stop()
                    break
                time.sleep(0.01)
        except Exception as e:
            logger.debug(f"Failed to play blocking {path}: {e}")

    def start_bg(self):
        """Start background music loop."""
        if not SOUND_ENABLED or SOUNDS.get("background") is None:
            return
        with self._lock:
            if not self.bg:
                try:
                    self.bg = sa.WaveObject.from_wave_file(str(SOUNDS["background"])).play()
                except Exception as e:
                    logger.debug(f"Failed to start background: {e}")
                    self.bg = None
    ...
    def stop_bg(self):
        ...
        with self._lock:
            try:
                ...
                logger.debug(f"Failed to stop background: {e}")
            finally:
                self.bg = None
...
def make_cap():
    """Create video capture with platform-specific optimizations."""
...
cap = make_cap()
if not cap.isOpened():
    raise SystemExit("[ERROR] Camera not accessible")
...
if DETECTOR == "yolo":
    try:
        ...
        if _HAS_MP:
            DETECTOR = "mp"
            logger.warning("Fallback to MediaPipe.")
        else:
            raise
...
if _HAS_MP:
    ...
    logger.info("Pinch-pointer hand control enabled.")
else:
    logger.warning("MediaPipe unavailable — keyboard only.")
...
class PointerController:
    ...
    def __init__(self):
        ...
        self.pinch_dist = 1.0
        self._cooldown = 0.0
        self.trail = deque(maxlen=TRAIL_MAX * 2)
        self.ripples = deque()
        self.hover_btn = None
        self.thumb_x = 0
        self.thumb_y = 0
    ...
    def update(self, frame):
        ...
        self.thumb_x = int(lm[4].x * w)
        self.thumb_y = int(lm[4].y * h)
        self.index_x = int(lm[8].x * w)
        self.index_y = int(lm[8].y * h)
        ...
        # Hysteresis pinch
        if self.pinching:
            if self.pinch_dist > PINCH_OFF_THRESH:
                self.pinching = False
        else:
            if self.pinch_dist < PINCH_ON_THRESH:
                self.pinching = True
    ...
    def draw(self, img):
        ...
        # ---- Trail ----
        alive_trail = [
            (x, y, t) for x, y, t in self.trail
            if now - t < TRAIL_DURATION
        ]
        self.trail.clear()
        self.trail.extend(alive_trail)
        for x, y, t in alive_trail:
            age = now - t
            a = 1.0 - age / TRAIL_DURATION
            r = max(1, int(3 * a))
        ...
        for rx, ry, rt in self.ripples:
            ...
            if age > RIPPLE_DURATION:
                continue
            alive_ripples.append((rx, ry, rt))
            rad = int(15 + age * 120)
            a = max(0, 1.0 - age / RIPPLE_DURATION)
            tk = max(1, int(3 * a))
            cv2.circle(img, (rx, ry), rad, (0, int(255 * a), 0), tk, cv2.LINE_AA)
        self.ripples.clear()
        self.ripples.extend(alive_ripples)
        ...
        if self.pinching:
            ...
            line_col = (0, int(min(255, 255 * (1 - pd / 0.3))), 255)
        else:
...
class HoverButton:
    """Interactive menu button with hover feedback."""
    ...
    def draw(self, img, is_selected=False):
        """Draw button with hover/selection feedback."""
        now = time.time()
        ...
        bc = (0, 255, 255) if is_selected else (
            (255, 255, 255) if self.is_hovered else (80, 80, 80)
        )
        tk = 2 if (is_selected or self.is_hovered) else 1
        cv2.rectangle(
            img, (self.x, self.y), (self.x + self.w, self.y + self.h),
            bc, tk, cv2.LINE_AA
        )
        (tw2, th2), _ = cv2.getTextSize(self.label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
        tx = self.x + (self.w - tw2) // 2
        ty = self.y + (self.h + th2) // 2
...
def draw_grace_overlay(img, gr):
    """Draw grace period overlay."""
    if gr <= 0:
        return
    h, w = img.shape[:2]
    a = min(0.45, gr / GRACE_PERIOD * 0.45)
    ov = img.copy()
...
def draw_fps(img, fps):
    """Draw FPS counter."""
    cv2.putText(
        img, f"FPS:{fps:.0f}", (10, img.shape[0] - 40),
        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 150), 1, cv2.LINE_AA
    )


def cur_green(ge):
    """Calculate current green phase duration."""
    t = min(1, ge / GAME_TIME)
    b = GREEN_MAX - t * (GREEN_MAX - GREEN_MIN)
    return max(GREEN_MIN * 0.5, b + random.uniform(-GREEN_VARIANCE, GREEN_VARIANCE))


def cur_red(ge):
    """Calculate current red phase duration."""
    t = min(1, ge / GAME_TIME)
    b = RED_MIN + t * (RED_MAX - RED_MIN)
    return max(RED_MIN * 0.5, b + random.uniform(-RED_VARIANCE, RED_VARIANCE))


def cur_thresh(ge):
    """Calculate current movement threshold."""
    t = min(1, ge / GAME_TIME)
    return TORSO_THRESH_EASY - t * (TORSO_THRESH_EASY - TORSO_THRESH_HARD)


def torso_cen(lm, w, h):
    """Get torso center and height from landmarks."""
    if DETECTOR != "mp" or mp_pose is None:
        return None, None
    ids = [
    ...
    try:
        pts = [(lm[i.value].x * w, lm[i.value].y * h) for i in ids]
    except Exception:
        return None, None
    cx = sum(p[0] for p in pts) / 4
    cy = sum(p[1] for p in pts) / 4
    torso_height = max(1, abs(pts[0][1] - pts[2][1]))
    return (cx, cy), torso_height
...
def bbox_lm(lm, w, h):
    ...
    return (
        max(0, min(xs) - BBOX_PAD),
        max(HEADER_HEIGHT, min(ys) - BBOX_PAD),
        min(w, max(xs) + BBOX_PAD),
        min(h, max(ys) + BBOX_PAD)
    )
...
def assign_all(dets):
    ...
    for di, (cx, cy, bb, th) in enumerate(dets):
        for tid, tr in trackers.items():
            ...
            if is_elim:
                dt *= ELIM_ABSORB_FACTOR  # wider radius for eliminated
            d = math.hypot(cx - tr["smooth_cx"], cy - tr["smooth_cy"])
            pr = 1 if is_elim else 0
            if d < dt:
                cands.append((pr, d, di, tid))
    cands.sort()
    ...
    for di in range(len(dets)):
        ...
        best_tid = None
        best_d = max(SPAWN_GUARD_MIN, th * SPAWN_GUARD_RATIO)
        for tid, tr in trackers.items():
            if tid in at:
                continue
            d = math.hypot(cx - tr["smooth_cx"], cy - tr["smooth_cy"])
        ...
        next_id += 1
        trackers[pid] = {
            "smooth_cx": cx, "smooth_cy": cy, "raw_cx": cx, "raw_cy": cy,
...
def merge_trackers():
    ...
    for i in range(len(ids)):
        ...
        for j in range(i + 1, len(ids)):
            ...
            mt = (ta["torso_h"] + tb["torso_h"]) / 2.0
            d = math.hypot(
                ta["smooth_cx"] - tb["smooth_cx"],
                ta["smooth_cy"] - tb["smooth_cy"]
            )
...
def cull():
    ...
    for k, v in trackers.items():
        ...
        if now - v["last_seen"] > to:
            dl.append(k)
    for k in dl:
        player_scores.pop(k, None)
        del trackers[k]
...
def check_movement(tid, ge):
    ...
    tr = trackers.get(tid)
    if not tr or tr.get("eliminated"):
        return False
    # Provisional immunity
    if tr["age_sec"] < NEW_TRACKER_GRACE_SEC:
        return False
    ...
    hist.append(moved)
    if len(hist) > MOVE_HISTORY_SIZE:
        hist = hist[-MOVE_HISTORY_SIZE:]
...
def draw_status(img, ge, gr=0, wa=0):
    ...
    h, w = img.shape[:2]
    tl = max(0, GAME_TIME - ge)
    if wa > 0:
        draw_freeze_border(img, wa)
    if state == STATE_GRACE and gr > 0:
        draw_grace_overlay(img, gr)
    ...
    cv2.putText(
        img, f"Alive: {al}  |  Out: {len(eliminated_set)}",
        (20, HEADER_HEIGHT + 72),
...
def draw_boxes(img):
    ...
    for tid, tr in trackers.items():
        ...
        if tr.get("eliminated"):
            ...
            bw3 = x2c - x1c
            bh3 = y2c - y1c
            fs = max(0.5, min(2.8, bw3 / 75, bh3 / 90))
            tk = max(2, int(fs * 2.5))
        ...
        if sc > 0:
            cv2.putText(
                img, f"{sc:.0f}pts", (x1 + 4, min(y2 + 18, h - 4)),
...
def draw_gameover(img, pe):
    ...
    draw_header(img)
    cv2.putText(
        img, "GAME OVER", (w // 2 - 160, h // 2 - 100),
        cv2.FONT_HERSHEY_DUPLEX, 1.8, col_red(), 4, cv2.LINE_AA
    )
    cv2.putText(
        img, f"Survivors: {survivors}", (w // 2 - 120, h // 2 - 50),
        cv2.FONT_HERSHEY_DUPLEX, 1.2, (255, 255, 255), 2, cv2.LINE_AA
    )
    cv2.putText(
        img, f"Eliminated: {len(eliminated_set)}", (w // 2 - 120, h // 2 - 10),
    ...
    if ss:
        ...
        for i, (tid, sc) in enumerate(ss):
            ...
            cv2.putText(
                img, f"{i + 1}. {tid}: {sc:.0f}pts{st}",
                (w // 2 - 120, h // 2 + 65 + i * 26),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, cl, 1, cv2.LINE_AA
            )
    rm = max(0, RESTART_DELAY - pe)
    cv2.putText(
        img, f"Restarting in {rm:.1f}s... (pinch to restart)",
        (w // 2 - 180, h - 25),
        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 150), 1, cv2.LINE_AA
    )
...
pointer.reset()
logger.info("Point & pinch to select. SPACE/Q for keyboard.")
logger.info(f"Mode: {CURRENT_MODE.upper()} | Detector: {DETECTOR.upper()}")
...
while True:
    ...
    if state == STATE_IDLE:
        ...
        if action:
            if action in GAME_MODES:
                ...
                _shapes_line = None
                _btn_init = False
                logger.info(f"Mode → {action.upper()}")
            elif action == "start":
                pointer.reset()
                start_game()
                continue
    ...
    # ==================== GREEN ====================
        ...
        if green_duration == 0:
            green_duration = cur_green(ge)
        tue = green_duration - pe
        if tue <= FREEZE_WARNING_PRE:
            wa = 1 - (tue / FREEZE_WARNING_PRE)
        if pe >= green_duration:
            green_duration = 0
            red_duration = cur_red(ge)
            transition(STATE_GRACE)
            audio.play_once(SOUNDS["red"])
        draw_boxes(frame)
        draw_header(frame)
        draw_status(frame, ge, wa=wa)
    ...
    # ==================== GRACE ====================
        ...
        if gr == 0:
            transition(STATE_RED)
        draw_boxes(frame)
        draw_header(frame)
        draw_status(frame, ge, gr=gr)
    ...
    if key == ord('q'):
        ...
        logger.info(f"Colorblind: {'ON' if COLORBLIND_MODE else 'OFF'}")
    elif key == ord('f'):
